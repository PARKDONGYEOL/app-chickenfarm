...
def get_current_settings():
    ...
    try:
        ...
        if system_instance is None:
            logger.warning("⚠️ 시스템 인스턴스가 아직 초기화되지 않음")
            return jsonify({
                'success': True,
                'data': default_settings,
                'message': '기본 설정값 (시스템 초기화 중)'
            })
        
        if not hasattr(system_instance, 'settings_manager'):
            logger.error("❌ 설정 관리자를 찾을 수 없음")
            return jsonify({
                'success': True,
                'data': default_settings,
                'message': '기본 설정값 (설정 관리자 없음)'
            })
        
        # 현재 설정 가져오기
        current_settings = system_instance.settings_manager.settings
        
        # 🔥 핵심: Decimal과 datetime을 JSON 직렬화 가능한 타입으로 변환
        settings_dict = {}
        ...
        logger.error(f"❌ 현재 설정 조회 실패: {e}", exc_info=True)
        
        # 에러 발생 시에도 기본값 반환 (500 에러 방지)
        return jsonify({
            'success': True,
            'data': default_settings,
            'message': f'설정 조회 실패, 기본값 사용: {str(e)}'
        })
